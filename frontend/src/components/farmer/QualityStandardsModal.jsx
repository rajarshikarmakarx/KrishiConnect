...
export default function QualityStandardsModal({
  ...
  const handleAddRule = () => {
    ...
    toast.success('New rule added. Customize its details below.', { icon: 'Γ₧ò' })
  }
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                >
                  ...
                  <span>{canEdit ? 'ΓÜÖ∩╕Å Officer Mode: Active' : 'Enable Officer Edit Mode'}</span>
                </button>
              </div>
                      ...
                      <table className="w-full text-left text-xs border-collapse min-w-[720px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                          {(activeData.grading_tiers || FALLBACK_DATA.grading_tiers).map((tier, idx) => {
                            const rawPayout = parseFloat(String(tier.payout_percentage).replace('%', ''))
                            const payoutMultiplier = !isNaN(rawPayout) ? rawPayout / 100 : (tier.payout_multiplier ?? 1.0)
                            const tierPrice = activeBaseMsp * payoutMultiplier
                            const penaltyAmount = activeBaseMsp - tierPrice
                            const penaltyPct = tier.penalty_percentage || (payoutMultiplier === 1 ? '0%' : payoutMultiplier <= 0 ? '-100%' : `-${Math.round((1 - payoutMultiplier) * 100)}%`)
                              ...
                              <tr key={tier.grade || idx} className="hover:bg-slate-50/50 dark:hover:bg-white/[0.02] transition-colors">
                                ...
                                <td className="p-3 whitespace-nowrap">
                                  <div className="flex flex-col items-start gap-0.5">
                                    <span className={`inline-flex items-center gap-1 font-black text-xs px-2 py-0.5 rounded-full border ${
                                      tier.color === 'emerald' ? 'text-emerald-700 dark:text-emerald-300 bg-emerald-100/90 dark:bg-emerald-500/20 border-emerald-300 dark:border-emerald-500/40' :
                                      tier.color === 'blue' ? 'text-blue-700 dark:text-blue-300 bg-blue-100/90 dark:bg-blue-500/20 border-blue-300 dark:border-blue-500/40' :
                                      tier.color === 'amber' ? 'text-amber-700 dark:text-amber-300 bg-amber-100/90 dark:bg-amber-500/20 border-amber-300 dark:border-amber-500/40' :
                                      'text-red-700 dark:text-red-300 bg-red-100/90 dark:bg-red-500/20 border-red-300 dark:border-red-500/40'
                                    }`}>
                                      {payoutMultiplier === 1 ? '0% Penalty' : payoutMultiplier <= 0 ? '-100% Blocked' : `${penaltyPct} Cut`}
                                    </span>
                                    <span className="text-[11px] font-bold text-slate-900 dark:text-white">
                                      ₹{formatNumber(tierPrice.toFixed(2))}/kg
                                    </span>
                                    <span className={`text-[10px] font-medium ${
                                      tier.color === 'emerald' ? 'text-emerald-700 dark:text-emerald-400' :
                                      tier.color === 'blue' ? 'text-blue-700 dark:text-blue-400' :
                                      tier.color === 'amber' ? 'text-amber-700 dark:text-amber-400' :
                                      'text-red-700 dark:text-red-400'
                                    }`}>
                                      {payoutMultiplier === 1 ? '100% MSP · No Deduction' :
                                       payoutMultiplier <= 0 ? 'Intake Prohibited (Total Loss)' :
                                       `-₹${formatNumber(penaltyAmount.toFixed(2))}/kg penalty (${tier.payout_percentage} Payout)`}
                                    </span>
                                  </div>
                                </td>
                    ...
                    {(activeData.grading_tiers || FALLBACK_DATA.grading_tiers).map((tier, idx) => {
                      ...
                      return (
                        <div key={tier.grade || idx} className={`bg-white dark:bg-[#0e1626] rounded-2xl border-2 ${borderColor} p-4 shadow-xs relative overflow-hidden`}>
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs bg-slate-50 dark:bg-white/5 rounded-xl p-2.5 border border-slate-200 dark:border-white/10">
                            <div>
                            ...
                            <div>
                              ...
                              <p className="font-black text-slate-900 dark:text-white text-sm">
                                {payoutMultiplier === 1 ? '0% Penalty' : payoutMultiplier <= 0 ? '-100% Loss' : `${penaltyPct} Cut`}
                              </p>
                              <span className={`text-[9px] font-bold block truncate ${
                                ...
                                {payoutMultiplier === 1 ? `100% Rate (₹${formatNumber(activeBaseMsp.toFixed(2))})` :
                                 payoutMultiplier <= 0 ? 'Intake Blocked (₹0)' :
                                 `-₹${formatNumber(penaltyAmount.toFixed(2))}/kg (${tier.payout_percentage})`}
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="bg-white dark:bg-[#0a101d] rounded-2xl p-4 border border-slate-200 dark:border-white/10 space-y-3 shadow-xs">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center text-xs">
                        {(activeData.grading_tiers || FALLBACK_DATA.grading_tiers).map((tier, idx) => {
                          ...
                          const cardBg =
                            tier.color === 'emerald' ? 'bg-emerald-50/70 dark:bg-emerald-950/30 border-emerald-200/80 dark:border-emerald-500/30' :
                            tier.color === 'blue' ? 'bg-blue-50/70 dark:bg-blue-950/30 border-blue-200/80 dark:border-blue-500/30' :
                            tier.color === 'amber' ? 'bg-amber-50/70 dark:bg-amber-950/30 border-amber-200/80 dark:border-amber-500/30' :
                            'bg-red-50/70 dark:bg-red-950/30 border-red-200/80 dark:border-red-500/30'

                          const titleColor =
                            tier.color === 'emerald' ? 'text-emerald-800 dark:text-emerald-300' :
                            tier.color === 'blue' ? 'text-blue-800 dark:text-blue-300' :
                            tier.color === 'amber' ? 'text-amber-800 dark:text-amber-300' :
                          ...
                          return (
                            <div key={tier.grade || idx} className={`p-2.5 rounded-xl border ${cardBg}`}>
                              <p className={`text-[10px] font-bold ${titleColor}`}>
                                {tier.tier_title?.split('(')[0]?.trim() || tier.grade}
                              </p>
                              <p className={`text-sm font-black mt-0.5 font-mono ${priceColor}`}>
                                ₹{formatNumber(tierPrice.toFixed(2))}/kg
                              </p>
                              <span className={`text-[10px] font-extrabold block mt-0.5 ${subColor}`}>
                                {payoutMultiplier === 1 ? '0% Penalty (100% MSP)' :
                                 payoutMultiplier <= 0 ? '-100% (Intake Blocked)' :
                                 `${penaltyPct} Cut (-₹${formatNumber(penaltyAmount.toFixed(2))})`}
                              </span>
                            </div>
                          )
                        })}
