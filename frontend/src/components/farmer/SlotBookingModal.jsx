...
export default function SlotBookingModal({ centre, onClose, onSuccess, onGoToQueue, activeQueueToken }) {
  ...
  const handleBook = async () => {
    ...
    if (!qty || parseFloat(qty) <= 0) return toast.error('Enter expected quantity in kg')
    setLoading(true)
    setActiveError(null)
    try {
  ...
  const handleCancelActive = async () => {
    ...
    try {
      ...
      toast.success('Previous booking cancelled. You can now book a new slot!')
      setActiveError(null)
      onGoToQueue?.(true) // refresh queue state
    } catch (e) {
      toast.error('Could not cancel existing booking')
    } finally {
      setCancelling(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between rounded-t-3xl sm:rounded-t-2xl">
          <div>
            <h2 className="font-bold text-slate-900">Book Procurement Slot</h2>
            <p className="text-sm text-slate-500">{centre.name}</p>
          </div>
          <button id="close-booking-modal" onClick={onClose} className="p-2 hover:bg-slate-100 rounded-xl transition-colors">
        ...
        <div className="p-5 space-y-5">
          ...
          {activeError && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-2xl space-y-3">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <h4 className="font-bold text-amber-900 text-sm">Active Booking Detected</h4>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button
                  ...
                  className="flex-1 py-2 px-3 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-semibold transition-colors"
                >
                  View Active Token
                </button>
                {activeQueueToken && (
                  <button
                    ...
                    disabled={cancelling}
                    className="py-2 px-3 bg-white hover:bg-red-50 text-red-600 border border-red-200 rounded-xl text-xs font-semibold transition-colors"
                  >
                    {cancelling ? 'Cancelling...' : 'Cancel & Re-book'}
                  </button>
                )}
              </div>
            </div>
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-1.5">
              <Wheat className="w-4 h-4 text-green-700" /> Select Crop
            </label>
          ...
          {/* Expected quantity */}
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2">Expected Quantity (kg)</label>
            <input
              id="input-quantity"
              type="number"
              min="1"
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2 flex items-center gap-1.5">
              <Clock className="w-4 h-4 text-green-700" /> Select Time Slot Today
            </label>
            {loadingSlots ? (
                ...
                {slots.map(slot => (
                  ...
                  >
                    ...
                    <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${
                      ...
                      {slot.is_full ? 'FULL' : `${slot.available} slots left`}
                    </span>
                  </button>
                ))}
              </div>
          ...
          >
            {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : '🎫 Confirm & Book Token'}
          </button>
        </div>
      </div>
    </div>
