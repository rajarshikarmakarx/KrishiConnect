...
import {
  ...
  Bot, User, ChevronDown, CheckCircle2, RotateCcw
} from 'lucide-react'
...
export default function KrishiChatbotModal({ user, onClose }) {
  ...
  const prepareSpeechText = (rawText) => {
    ...
    return rawText
      ...
      .replace(/\|[\s\-:|]+\|/g, ' ')
      // Replace table pipes with commas for natural speech pause
      .replace(/\|/g, ', ')
      // Remove markdown dividers ---, ***, ___
      .replace(/^[-*_]{3,}$/gm, ' ')
  ...
  const speakText = (text, index) => {
    ...
    // Split into natural spoken sentence chunks (Bengali Dāri ।, !, ?, .)
    ...
    // If chunking didn't produce chunks, fallback to slicing
    if (chunks.length === 0) {
    ...
    // 3. For Bengali and Hindi without native Windows voices, stream via backend proxy
    ...
    const playChunk = () => {
      ...
      audio.onerror = () => {
        // Skip errored chunk and continue next
        chunkIdx++
        playChunk()
      }

      audio.play().catch(() => {
        // If autoplay blocks sequential play, reset state
        setSpeakingIndex(null)
      })
    }
