"""
Dynamic Geographic Distance & Road Routing Engine
Calculates driving distance and travel duration between geographic coordinates.
Attempts OSRM driving route with short timeout (~1.8s) and in-memory caching.
...
# In-memory routing cache: (lat1, lon1, lat2, lon2) -> {"distance_km": float, "duration_minutes": float, "source": str}
        ...
        with urllib.request.urlopen(req, timeout=timeout_seconds) as response:
            if response.status == 200:
                ...
                if data.get("code") == "Ok" and data.get("routes"):
                    ...
                    # Farm vehicles travel slower than highway cars; scale or use 3.0 min/km
...
async def calculate_distance_and_duration(
    ...
    First checks in-memory cache, then attempts OSRM routing, with transparent
    ...
    if not force_fallback and cache_key in _ROUTING_CACHE:
        cached = _ROUTING_CACHE[cache_key].copy()
        cached["source"] = "cache"
        return cached
    ...
    try:
        ...
        if osrm_result is not None:
            _ROUTING_CACHE[cache_key] = osrm_result
            return osrm_result
    except Exception:
        pass
    ...
    _ROUTING_CACHE[cache_key] = fallback_result
