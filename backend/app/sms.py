...
def _format_sms_text(msg_type: str, lang: str, params: Dict[str, Any]) -> str:
    ...
    if msg_type == "OTP":
        ...
        templates = {
            ...
            "te": f"కృషికనెక్ట్ OTP: {otp}. మీ రైతు ఖాతాను ధృవీకరించడానికి ఈ కోడ్‌ను ఉపయోగించండి. గడువు 10 నిమిషాలు. Govt of India DoCA.",
        ...
        templates = {
            "bn": f"কৃষিকানেক্ট: {crop} সংগ্রহ সম্পন্ন। ওজন: {qty} কেজি, দর: ₹{rate}/কেজি। মোট: ₹{amount}। PFMS মাধ্যমে DBT পেমেন্ট ব্যাঙ্কে পাঠানো হয়েছে।",
            "hi": f"कृषिकनेक्ट: {crop} खरीद पूरी हुई। वजन: {qty} kg, दर: ₹{rate}/kg। कुल: ₹{amount}। PFMS द्वारा डीबीटी भुगतान बैंक खाते में भेजा गया।",
            "mr": f"कृषिकनेक्ट: {crop} खरेदी पूर्ण झाली. निव्वळ वजन: {qty} किलो, दर: ₹{rate}/किलो. एकूण: ₹{amount}. PFMS द्वारे थेट DBT बँकेत पाठवले.",
            "te": f"కృషికనెక్ట్: {crop} సేకరణ పూర్తయింది. నికర బరువు: {qty} కిలోలు, ధర: ₹{rate}/కిలో. మొత్తం: ₹{amount}. PFMS ద్వారా DBT చెల్లింపు ప్రారంభమైంది.",
            "ta": f"கிருஷிகனெக்ட்: {crop} கொள்முதல் முடிந்தது. எடை: {qty} கிலோ, விலை: ₹{rate}/கிலோ. மொத்தம்: ₹{amount}. PFMS மூலம் DBT பணம் வங்கிக் கணக்கிற்கு அனுப்பப்பட்டது.",
            "gu": f"કૃષિકનેક્ટ: {crop} ખરીદી પૂર્ણ થઈ. ચોખ્ખું વજન: {qty} કિલો, ભાવ: ₹{rate}/કિલો. કુલ: ₹{amount}. PFMS દ્વારા DBT બેંકમાં મોકલાયું.",
            "kn": f"ಕೃಷಿಕನೆಕ್ಟ್: {crop} ಖರೀದಿ ಪೂರ್ಣಗೊಂಡಿದೆ. ತೂಕ: {qty} ಕೆಜಿ, ದರ: ₹{rate}/ಕೆಜಿ. ಒಟ್ಟು: ₹{amount}. PFMS ಮೂಲಕ ನೇರ DBT ಪಾವತಿ ಬ್ಯಾಂಕಿಗೆ ಕಳುಹಿಸಲಾಗಿದೆ.",
            "ml": f"കൃഷികണക്ട്: {crop} സംഭരണം പൂർത്തിയായി. ഭാരം: {qty} കിലോഗ്രാം, നിരക്ക്: ₹{rate}/കിലോഗ്രാം. ആകെ: ₹{amount}. PFMS വഴി DBT തുക ബാങ്കിലേക്ക് അയച്ചു.",
            "pa": f"ਕ੍ਰਿਸ਼ੀਕਨੈਕਟ: {crop} ਖ਼ਰੀਦ ਮੁਕੰਮਲ ਹੋਈ। ਵਜ਼ਨ: {qty} ਕਿਲੋ, ਰੇਟ: ₹{rate}/ਕਿਲੋ। ਕੁੱਲ: ₹{amount}। PFMS ਰਾਹੀਂ ਸਿੱਧਾ DBT ਭੁਗਤਾਨ ਬੈਂਕ ਖਾਤੇ ਵਿੱਚ ਭੇਜਿਆ ਗਿਆ।",
            "or": f"କୃଷିକନେକ୍ଟ: {crop} କ୍ରୟ ସମ୍ପୂର୍ଣ୍ଣ ହେଲା। ଓଜନ: {qty} କି.ଗ୍ରା., ଦର: ₹{rate}/କି.ଗ୍ରା.। ମୋଟ: ₹{amount}। PFMS ମାଧ୍ୟମରେ DBT ରାଶି ବ୍ୟାଙ୍କ ଖାତାକୁ ପଠାଗଲା।",
            "as": f"কৃষিকনেক্ট: {crop} ক্ৰয় সম্পূৰ্ণ হ'ল। ওজন: {qty} কি.গ্ৰা., দৰ: ₹{rate}/কি.গ্ৰা.। মুঠ: ₹{amount}। PFMS যোগে DBT ধন বেংক একাউন্টলৈ প্ৰেৰণ কৰা হ'ল।",
            "en": f"KrishiConnect: {crop} procurement complete. Net: {qty} kg @ Rs {rate}/kg. Total: Rs {amount}. Direct DBT payment initiated via PFMS."
        }
        return templates.get(lang, templates["en"])
